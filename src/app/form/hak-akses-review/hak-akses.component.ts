...
export class HakAksesComponent implements OnInit {
  ...
  fetchDataFormHA() {
    axios
      .get(`${environment.apiUrl2}/hak/akses/review`)
      .then((response) => {
  ...
  fetchDataAdminFormHA() {
    axios
      .get(`${environment.apiUrl2}/admin/my/ha/review/division`, {
        headers: {
  ...
  fetchDataUserFormHA() {
    axios
      .get(`${environment.apiUrl2}/api/my/form/ha/review`, {
        headers: {
          Authorization: `Bearer ${this.cookieService.get('userToken')}`,
        },
      })
  ...
  addFormHA() {
    ...
    axios
      .post(
        `${environment.apiUrl2}/api/add/ha/review`,
        {
          isPublished: false,
          formData: {
            document_uuid: this.document_uuid,
  ...
  openEditModal(form_uuid: string) {
    axios
      .get(`${environment.apiUrl2}/ha/review/${form_uuid}`)
      .then((response) => {
        // Buka modal edit
        ...
        console.log(
          'Data info HA setelah di-set:',
          this.infoHAListArray().getRawValue()
        );
      })
      .catch((error) => {
  ...
  updateFormHA() {
    ...
    axios
      .put(
        `${environment.apiUrl2}/api/hak/akses/review/update/${this.form_uuid}`,
        data,
        {
          headers: {
            Authorization: `Bearer ${this.cookieService.get('userToken')}`,
  ...
  openPreviewPage(form_uuid: any) {
    axios
      ...
      .then((response) => {
        ...
        this.router.navigate([`/form/ha/${form_uuid}`]);
      })
      .catch((error) => {
        if (error.response) {
  ...
  closePreviewPage() {
    ...
    this.router.navigate([`/form/hak-akses`]);
  }
  ...
  generateURL(form_uuid: string) {
    // Generate URL
    const url = `${window.location.origin}/form/ha/${form_uuid}`;
